...
name: simplifier
description: >
    Simplify, clean up, and refine PHP/Laravel code for clarity, consistency, and maintainability
    while preserving all functionality. Use this skill when the user asks to clean up code, reduce
    complexity, refactor for readability, remove duplication, or improve code quality — even if they
    don't explicitly say "simplify". Also activates after new code has been written and could benefit
    from a consistency pass. Does not change behaviour; only improves how the code is expressed.
---
...
4. **Maintain Balance**: Avoid oversimplification that could:
