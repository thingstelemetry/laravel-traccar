<?php

declare(strict_types=1);

namespace ThingsTelemetry\Traccar\Requests\Report;

use Saloon\Enums\Method;
use Saloon\Http\Request;
use Saloon\Http\Response;
use Carbon\CarbonImmutable;
use Illuminate\Support\Collection;
use ThingsTelemetry\Traccar\Dto\ReportGeofencesData;

class GetGeofencesReport extends Request
{
    protected Method $method = Method::GET;

    /**
     * @param  array<int>  $deviceIds
     * @param  array<int>  $groupIds
     * @param  array<int>|null  $geofenceIds
     */
    public function __construct(
        public array $deviceIds,
        public array $groupIds,
        public CarbonImmutable $from,
        public CarbonImmutable $to,
        public ?array $geofenceIds = null,
    ) {
    }

    public function resolveEndpoint(): string
    {
        return '/reports/geofences';
    }

    /** @return Collection<int, ReportGeofencesData> */
    public function createDtoFromResponse(Response $response): Collection
    {
        return collect(value: $response->json())
            ->map(callback: fn (array $report) => ReportGeofencesData::fromArray(data: $report));
    }

    /** @return array<string, mixed> */
    protected function defaultQuery(): array
    {
        return array_filter([
            'deviceId'   => $this->deviceIds,
            'groupId'    => $this->groupIds,
            'geofenceId' => $this->geofenceIds,
            'from'       => $this->from->toIso8601String(),
            'to'         => $this->to->toIso8601String(),
        ], static fn (mixed $value): bool => $value !== null && $value !== []);
    }
}
